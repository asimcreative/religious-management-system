...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('teachers.show', $teacher) }}" class="btn btn-outline-secondary btn-sm">
        <i class="bi bi-arrow-left"></i> {{ __('teachers.back_to_detail') }}
    </a>
</div>
...
<form method="POST" action="{{ route('teachers.update', $teacher) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="employee_id" id="employee_id" class="form-select @error('employee_id') is-invalid @enderror" required>
                        ...
                        {{-- Include current employee even if already linked --}}
                        @if($teacher->employee)
                            <option value="{{ $teacher->employee_id }}" selected>
                                {{ $teacher->employee->employee_code }} — {{ $teacher->employee->employee_name }}
                            </option>
                        @endif
                        @foreach($employees as $emp)
                            @if($emp->id !== $teacher->employee_id)
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                        @foreach(App\Enums\Status::cases() as $status)
                            <option value="{{ $status->value }}" {{ old('status', $teacher->status->value) == $status->value ? 'selected' : '' }}>{{ $status->label() }}</option>
                        @endforeach
                    </select>
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            ...
            @php $currentBranchIds = old('branch_ids', $teacher->branches->pluck('id')->toArray()); @endphp
            <div class="row g-2">
                @foreach($branches as $id => $name)
                    <div class="col-md-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="branch_ids[]" value="{{ $id }}"
                                   id="branch_{{ $id }}" {{ in_array($id, $currentBranchIds) ? 'checked' : '' }}>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            <textarea name="notes" id="notes" rows="3" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $teacher->notes) }}</textarea>
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('teachers.show', $teacher) }}" class="btn btn-outline-secondary">{{ __('teachers.cancel') }}</a>
    </div>
</form>
@endsection
