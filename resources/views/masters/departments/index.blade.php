...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">{{ __('masters.departments') }}</h4>
    @can('create', App\Models\Department::class)
        <a href="{{ route('masters.departments.create') }}" class="btn btn-primary btn-sm">
                        ...
                        <tr>
                            ...
                            <td>
                                @can('update', $department)
                                    <a href="{{ route('masters.departments.edit', $department) }}" class="btn btn-outline-primary btn-sm">{{ __('masters.edit') }}</a>
                                @endcan
                                @can('delete', $department)
                                    <form method="POST" action="{{ route('masters.departments.destroy', $department) }}" class="d-inline"
