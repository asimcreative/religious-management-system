...
@section('title', __('masters.edit') . ' ' . __('masters.attendance_reason'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">{{ __('masters.edit') }} {{ __('masters.attendance_reason') }}</h4>
    <a href="{{ route('masters.attendance-reasons.index') }}" class="btn btn-outline-secondary btn-sm">{{ __('masters.back') }}</a>
</div>

<div class="card">
    <div class="card-body">
        <form method="POST" action="{{ route('masters.attendance-reasons.update', $reason) }}">
            @csrf @method('PUT')
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="reason_name" class="form-control @error('reason_name') is-invalid @enderror"
                           value="{{ old('reason_name', $reason->reason_name) }}" required>
                ...
                <div class="col-md-3">
                    ...
                    <input type="color" name="color" class="form-control form-control-color @error('color') is-invalid @enderror"
                           value="{{ old('color', $reason->color ?? '#6c757d') }}">
                    @error('color') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-3">
                    <label class="form-label">{{ __('masters.icon') }}</label>
                    <input type="text" name="icon" class="form-control @error('icon') is-invalid @enderror"
                           value="{{ old('icon', $reason->icon) }}" placeholder="bi bi-check-circle">
                ...
                <div class="col-md-3">
                    <div class="form-check mt-4">
                        ...
                        <input class="form-check-input" type="checkbox" name="counts_as_absent" value="1" id="counts_as_absent"
                               {{ old('counts_as_absent', $reason->counts_as_absent) ? 'checked' : '' }}>
                ...
                <div class="col-md-3">
                    <div class="form-check mt-4">
                        ...
                        <input class="form-check-input" type="checkbox" name="counts_as_leave" value="1" id="counts_as_leave"
                               {{ old('counts_as_leave', $reason->counts_as_leave) ? 'checked' : '' }}>
                ...
                <div class="col-md-4">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="1" {{ old('status', $reason->status->value) == 1 ? 'selected' : '' }}>{{ __('masters.active') }}</option>
                        <option value="0" {{ old('status', $reason->status->value) == 0 ? 'selected' : '' }}>{{ __('masters.inactive') }}</option>
                    </select>
