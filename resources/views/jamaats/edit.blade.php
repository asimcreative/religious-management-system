            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" id="jamaat_number" name="jamaat_number" class="form-control @error('jamaat_number') is-invalid @enderror"
                           value="{{ old('jamaat_number', $jamaat->jamaat_number) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" id="jamaat_name" name="jamaat_name" class="form-control @error('jamaat_name') is-invalid @enderror"
                           value="{{ old('jamaat_name', $jamaat->jamaat_name) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <select id="status" name="status" class="form-select @error('status') is-invalid @enderror" required>
                        @foreach(App\Enums\Status::cases() as $status)
                            <option value="{{ $status->value }}" {{ old('status', $jamaat->status->value) == $status->value ? 'selected' : '' }}>{{ $status->label() }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <select id="branch_id" name="branch_id" class="form-select @error('branch_id') is-invalid @enderror" required>
                        ...
                        @foreach($branches as $id => $name)
                            <option value="{{ $id }}" {{ old('branch_id', $jamaat->branch_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select id="leader_id" name="leader_id" class="form-select @error('leader_id') is-invalid @enderror" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('leader_id', $jamaat->leader_id) == $employee->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <select id="vice_leader_id" name="vice_leader_id" class="form-select @error('vice_leader_id') is-invalid @enderror">
                        <option value="">{{ __('jamaats.select') }}</option>
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('vice_leader_id', $jamaat->vice_leader_id) == $employee->id ? 'selected' : '' }}>
                                {{ $employee->employee_name }} ({{ $employee->employee_code }})
                            </option>
                        @endforeach
                    </select>
